...
# Training
model = tflearn.DNN(net, checkpoint_path='./checkpoints/resnet_sat6', max_checkpoints=1, tensorboard_verbose=3)
...
#print(P)
for Y_t in y_test:
    T.append(np.nonzero(Y_t)[0])
#Z=zip(P,T)
#for z in Z:
 #   print(z[0],z[1])
#print(tflearn.contrib.metrics.confusion_matrix(y_test, P,10))
#print(np.array(P))
conf_arr=confusion_matrix(np.array(T),np.array(P))
norm_conf = []
for i in conf_arr:
    a = 0
    tmp_arr = []
